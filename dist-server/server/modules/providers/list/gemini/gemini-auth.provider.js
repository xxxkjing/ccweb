...
export class GeminiProviderAuth {
    /**
    ...
    async checkCredentials() {
        ...
        if (selectedType === 'vertex-ai') {
            ...
            const hasProject = Boolean(process.env.GOOGLE_CLOUD_PROJECT?.trim()
                || process.env.GOOGLE_CLOUD_PROJECT_ID?.trim()
                || readOptionalString(userEnv.GOOGLE_CLOUD_PROJECT)
                || readOptionalString(userEnv.GOOGLE_CLOUD_PROJECT_ID));
            const hasLocation = Boolean(process.env.GOOGLE_CLOUD_LOCATION?.trim()
     ...
     */
    async getTokenInfoEmail(accessToken) {
        try {
...
//# sourceMappingURL=gemini-auth.provider.js.map