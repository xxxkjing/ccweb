...
import { findFilesRecursivelyCreatedAfter, normalizeProjectPath, normalizeSessionName, readFileTimestamps, } from '../../../../shared/utils.js';
/**
 * Session indexer for Gemini transcript artifacts.
 */
export class GeminiSessionSynchronizer {
    provider = 'gemini';
    ...
    async synchronize(since) {
        ...
        //   since ?? null
        // );
        const jsonlTempFiles = await findFilesRecursivelyCreatedAfter(path.join(this.geminiHome, 'tmp'), '.jsonl', since ?? null);
     ...
     */
    async processLegacySessionFile(filePath) {
        try {
     ...
     */
    async processJsonlSessionFile(filePath, projectHashLookup) {
     ...
     */
    async extractJsonlMetadata(filePath) {
        try {
            const content = await readFile(filePath, 'utf8');
     ...
     */
    async resolveProjectPathFromChatWorkspace(filePath) {
     ...
     */
    addProjectHashCandidates(lookup, projectPath) {
     ...
     */
    extractGeminiTextContent(content) {
     ...
     */
    shouldSkipTempArtifact(filePath) {
        return (filePath.startsWith(path.join(this.geminiHome, 'tmp'))
...
//# sourceMappingURL=gemini-session-synchronizer.provider.js.map